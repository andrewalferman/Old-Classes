...
# Density of ethanol
Methanol = 46.06844 * ureg.gram / ureg.mol
...
fuelmass = 38 * ureg.gram # Measured value
potmeasurement = 99 * ureg.gram # Measured value
watermassmeasurement = 590 * ureg.gram # Measured value
ethanolmass = 10000 * ureg.gram # Dummy value because 100 percent ethanol
bottletotal = 10000 * ureg.gram # Dummy value because 100 percent ethanol
hotwatermass = 436 * ureg.gram # Measured value

# Poured volume of fuel in liters
fuelvolume = 50 * ureg.milliliter # Measured value

# Temperature measurements
starttemp = 301.15 * ureg.kelvin # Measured value
tempreading = Q_(75.3, ureg.degF) # Measured value
...
pressurediff = 0.459065 * ureg.inch_H2O_39F # Measured value

# Absolute pressure in Pascals
barpressure = 1024. * ureg.millibar # Measured value

# Exhaust gas constituent measurements
# Points 140 to 531 were measured
COlevel = 15.5732 * 1.e-6 * ureg.mol/ureg.mol # Measured value
CO2level = 316.2379 * 1.e-6 * ureg.mol/ureg.mol # Measured value

# Add in burn time, in seconds
minutes = 20. * ureg.minute # Measured value
seconds = 12. * ureg.second # Measured value
...
burntime = (minutes + seconds).plus_minus(1. * ureg.second).to(ureg.second)
...
mfuel = (fuelv * ethrho100).to(ureg.kilogram)
...
nethanol = (NA * mfuel * Cx / Methanol).to('')
...
print('-----------------------------')
print('NOMINAL COMBUSTION EFFICIENCY')
print('Nominal value: {:.2f}'.format(nce.n * 100.))
print('Relative uncertainty: +/- {:.2f}%'.format(nce.rel * 100.))
