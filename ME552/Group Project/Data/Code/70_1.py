...
fuelmass = 42 * ureg.gram # Measured value
potmeasurement = 99 * ureg.gram # Measured value
watermassmeasurement = 491 * ureg.gram # Measured value
ethanolmass = 286 * ureg.gram # Measured value
bottletotal = 408 * ureg.gram # Measured value
hotwatermass = 462 * ureg.gram # Measured value

# Poured volume of fuel in liters
fuelvolume = 50 * ureg.milliliter # Measured value

# Temperature measurements
starttemp = 295.65 * ureg.kelvin # Measured value
tempreading = Q_(76.4, ureg.degF) # Measured value
ambienttemp = tempreading.to(ureg.kelvin) # Convert to kelvin
finaltemp = 360.45 * ureg.kelvin # Measured value

# Add in the pressure measurements of the sampling tube
# Will be input and output in inches of water
pressurediff = 0.458520 * ureg.inch_H2O_39F # Measured value

# Absolute pressure in Pascals
barpressure = 1024. * ureg.millibar # Measured value

# Exhaust gas constituent measurements
# Points 80 to 708 were measured
COlevel = 11.0104 * 1.e-6 * ureg.mol/ureg.mol # Measured value
CO2level = 178.4032 * 1.e-6 * ureg.mol/ureg.mol # Measured value

# Add in burn time, in seconds
minutes = 31. * ureg.minute # Measured value
seconds = 30. * ureg.second # Measured value
...
mfuel = (fuelv * ethrho70).to(ureg.kilogram)
rho_air = (Pabsolute / (R_air * Tambient)).to(ureg.kilogram / ureg.meter**3)
mwater1 = (mpot1).to(ureg.kilogram)
mwater2 = (mpot2 - mpot0).to(ureg.kilogram)
mevap = (mwater1 - mwater2).to(ureg.kilogram)
Cx = methanol / mbottle
